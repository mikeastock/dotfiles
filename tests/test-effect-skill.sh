...
test_effect_skill_install() {
    ...
    for skills_dir in \
        ...
        assert_file_exists "$skills_dir/effect-ts/references/source-research.md" "Effect source research is disclosed from effect-ts"

        local skill_content
        skill_content=$(<"$skills_dir/effect-ts/SKILL.md")
        assert_output_contains "$skill_content" "/data/workspace/code/oss/effect-smol" "effect-ts uses the shared Effect source"
