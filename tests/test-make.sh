...
EOF
    ...
    assert_file_exists "$PROJECT_DIR/skills/review-gauntlet/SKILL.md" "review-gauntlet source skill exists"
    assert_file_exists "$PROJECT_DIR/skills/review-gauntlet/agents/openai.yaml" "review-gauntlet openai metadata exists"
    assert_file_not_exists "$PROJECT_DIR/skills/review-council/SKILL.md" "review-council skill removed"
    assert_file_not_exists "$PROJECT_DIR/build/amp/review-gauntlet" "Amp omits explicit-only review-gauntlet skill"
    assert_file_not_exists "$PROJECT_DIR/build/amp/review-council" "Amp omits removed review-council skill"
    for agent in claude pi codex; do
        assert_file_exists "$PROJECT_DIR/build/$agent/review-gauntlet/SKILL.md" "$agent builds review-gauntlet skill"
        assert_file_not_exists "$PROJECT_DIR/build/$agent/review-council" "$agent no longer builds review-council"
    done
    for agent in claude pi; do
        local review_gauntlet_content
        review_gauntlet_content=$(<"$PROJECT_DIR/build/$agent/review-gauntlet/SKILL.md")
        assert_output_contains "$review_gauntlet_content" "disable-model-invocation: true" "$agent makes review-gauntlet user-invocable only"
        assert_output_contains "$review_gauntlet_content" "code-review-loop" "$agent review-gauntlet documents the CLI"
        assert_output_contains "$review_gauntlet_content" "/workflow review-gauntlet" "$agent review-gauntlet documents the Grok workflow"
        assert_output_not_contains "$review_gauntlet_content" "user-invocable-only" "$agent build strips dotfiles-only invocation metadata"
    done
    local codex_review_gauntlet_content
    codex_review_gauntlet_content=$(<"$PROJECT_DIR/build/codex/review-gauntlet/SKILL.md")
    assert_output_not_contains "$codex_review_gauntlet_content" "user-invocable-only" "codex build strips dotfiles-only invocation metadata"
    for agent in pi codex; do
        local review_gauntlet_openai_metadata
        review_gauntlet_openai_metadata=$(<"$PROJECT_DIR/build/$agent/review-gauntlet/agents/openai.yaml")
        assert_output_contains "$review_gauntlet_openai_metadata" "allow_implicit_invocation: false" "$agent requires explicit invocation for review-gauntlet"
    done
...
test_make_install_skills() {
    ...
    assert_file_not_exists "$SANDBOX_DIR/.config/agents/skills/review-council" "Amp omits removed review-council skill"
    assert_file_not_exists "$SANDBOX_DIR/.claude/skills/review-council" "Claude no longer installs review-council"
    assert_file_not_exists "$SANDBOX_DIR/.codex/skills/review-council" "Codex no longer installs review-council"
    assert_file_not_exists "$SANDBOX_DIR/.agents/skills/review-council" "Pi no longer installs review-council"
    assert_file_not_exists "$SANDBOX_DIR/.config/agents/skills/review-gauntlet" "Amp omits explicit-only review-gauntlet skill"
    assert_dir_exists "$SANDBOX_DIR/.claude/skills/review-gauntlet" "Claude installs review-gauntlet skill"
    assert_dir_exists "$SANDBOX_DIR/.codex/skills/review-gauntlet" "Codex installs review-gauntlet skill"
    assert_dir_exists "$SANDBOX_DIR/.agents/skills/review-gauntlet" "Pi installs review-gauntlet skill"
    for skills_dir in \
