...
digraph commit_workflow {
    ...
    deviations [label="Implementation\nDeviation Report"];
...
### 4. Implementation Deviation Report

If this commit completes or materially advances work from a design doc, plan, issue, or explicit requirements, compare the staged diff against that source before committing:

- What did the implementation do differently than planned?
- What decisions did the implementation make that the source did not cover?
- Did the implementation add or remove scope?
- Should the design, plan, or docs be updated before this commit?

If there is no written source of truth because the task is purely mechanical, note that and continue.

### 5. Determine Type and Scope
