...
- Before implementation on architecture-heavy work, request `architecture-reviewer` against the design/plan before requesting code review against a diff.
...
- Dispatch `architecture-reviewer` after the design doc is written and before the implementation plan is final
- Re-run it when implementation materially deviates from the design
- Use `code-reviewer` after code exists
