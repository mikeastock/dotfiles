...
Public package and module surface area to be aware of when researching or implementing a solution. Each module entry includes its checked-out repo path so it can be located quickly.
...
- No public `src/index.ts` barrel is present in this checked-out repo, so there are no barrel exports to inventory here.
