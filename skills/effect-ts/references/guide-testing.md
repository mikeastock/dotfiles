...
This guide is based on the checked-out `@effect/vitest` package in `/data/workspace/code/oss/effect-smol`.
...
- `/data/workspace/code/oss/effect-smol/packages/vitest/test/index.test.ts`
- `/data/workspace/code/oss/effect-smol/packages/vitest/typetest/index.tst.ts`
...
The checked-out repo also uses `expect` in some tests, but for skill guidance prefer `assert` in Effect-based tests because it keeps tests more uniform and explicit inside Effect programs.
...
Examples from the checked-out tests show these are first-class parts of the API.
...
Unlike top-level `it.prop`, array-form `it.effect.prop` supports `Schema` inputs by converting them with `Schema.toArbitrary`.
The checked-out implementation does not currently perform that conversion for object-form arbitrary records, so use explicit `FastCheck` arbitraries there.

So prefer `it.effect.prop` when:

- the property test needs `Effect`
- you want to use `Schema` values in an array-form arbitrary list
...
The checked-out tests explicitly verify resource release through `afterAll`.
