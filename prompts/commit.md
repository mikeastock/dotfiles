...
4. Write a Conventional Commit message that explains what changed (see global agent guidelines on semantic commits).
5. Run the narrowest relevant verification if it has not already been run.
6. Commit with a non-interactive editor environment, for example `GIT_EDITOR=true git commit`.
