...
.PHONY: all install install-non-interactive install-tools setup-tools install-skills install-amp-plugins install-extensions install-prompts install-themes install-configs amp-plugin-types amp-plugin-check package-manager-security-config build clean help submodule-init plugin-update check-python \
...
help:
	...
	@echo "  make setup-tools             Configure installed external agent tools"
...
setup-tools: check-python
	@$(PYTHON) "$(BUILD_SCRIPT)" setup-tools
