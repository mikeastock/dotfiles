...
### CI/CD
GitHub Actions (`.github/workflows/test.yml`) runs `./tests/run-all.sh`, `pnpm test`, `pnpm typecheck`, and the tests/typecheck of each vendored bb plugin (`bb-plugins/bb-plugin-t3sidebar`, `bb-plugins/bb-plugin-bb-sidebar`, `bb-plugins/bb-plugin-tok-speed`) on push/PR to main/master.
...
- Dependencies are self-documented in the script
- uv handles environment creation and caching automatically
- Scripts remain portable and reproducible
