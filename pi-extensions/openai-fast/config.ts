...
const LEGACY_DEFAULT_SUPPORTED_MODEL_KEYS = ["openai/gpt-5.4", "openai-codex/gpt-5.4", "openai-codex/gpt-5.5"] as const;
...
function ensureGlobalConfigFile(globalConfigPath: string): void {
	...
	const config = readConfigFile(globalConfigPath);
	if (
		config?.supportedModels &&
		config.supportedModels.length === LEGACY_DEFAULT_SUPPORTED_MODEL_KEYS.length &&
		LEGACY_DEFAULT_SUPPORTED_MODEL_KEYS.every((model) => config.supportedModels?.includes(model))
	) {
		writeConfigFile(globalConfigPath, { ...config, supportedModels: [...DEFAULT_SUPPORTED_MODEL_KEYS] });
	}
}
